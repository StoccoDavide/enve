/**
 * sfun_ENVE_SIMULATOR.cpp
 *
 *    ABSTRACT:
 *      The purpose of this sfunction is to call a simple legacy
 *      function during simulation:
 *
 *         void sfun_out(shellsRF      u1[1], groundContact y1[1], double        p13[1], double        p14[1])
 *
 *    Simulink version           : 10.4 (R2021b) 14-May-2021
 *    C++ source code generated on : 22-Nov-2021 09:58:38
 *
...
/**
     ...
     def.OutputFcnSpec = 'void sfun_out(shellsRF      u1[1], groundContact y1[1], double        p13[1], double        p14[1])';
     def.StartFcnSpec = 'void sfun_init(double p1[1], double p2[1], double p3[1], double p4[1], double p5[1], double p6[1], double p7[1], double p8[1], double p9[1], double p10[1], double p11[1], double p12[1])';
...
#define S_FUNCTION_NAME  sfun_ENVE_SIMULATOR
...
static void mdlInitializeSizes(SimStruct *S)
{
    ...
    if (ssRTWGenIsCodeGen(S) || ssGetSimMode(S)==SS_SIMMODE_EXTERNAL) {
        ...
        if (isSimulationTarget) {
            ...
            ssRegMdlInfo(S, "sfun_ENVE_SIMULATOR_wrapper_allocmem", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));

            /* Register reserved identifier for freeing PWork for SimulationTarget */
            ssRegMdlInfo(S, "sfun_ENVE_SIMULATOR_wrapper_freemem", MDL_INFO_ID_RESERVED, 0, 0, ssGetPath(S));
        } /* if */
    } /* if */
...
static void mdlStart(SimStruct *S)
{
    if (IS_SIMULATION_TARGET(S)) {
        ...
        /* Get information for accessing groundContact.rho_dot_rr */
        __dtBusInfo[24] = ssGetBusElementOffset(S, __groundContactId, 12);
        __dtBusInfo[25] = __dtSizeInfo[1];

        /* Get information for accessing groundContact.rho_dot_rl */
        __dtBusInfo[26] = ssGetBusElementOffset(S, __groundContactId, 13);
        __dtBusInfo[27] = __dtSizeInfo[1];

        /* Get information for accessing groundContact.rho_dot_fr */
        __dtBusInfo[28] = ssGetBusElementOffset(S, __groundContactId, 14);
        __dtBusInfo[29] = __dtSizeInfo[1];

        /* Get information for accessing groundContact.rho_dot_fl */
        __dtBusInfo[30] = ssGetBusElementOffset(S, __groundContactId, 15);
        __dtBusInfo[31] = __dtSizeInfo[1];

        /* Get information for accessing groundContact.in_mesh */
        __dtBusInfo[32] = ssGetBusElementOffset(S, __groundContactId, 16);
        __dtBusInfo[33] = __dtSizeInfo[1];

        /* Get information for accessing shellsRF.RFw_rr */
        __dtBusInfo[34] = ssGetBusElementOffset(S, __shellsRFId, 0);
        __dtBusInfo[35] = 16*__dtSizeInfo[1];

        /* Get information for accessing shellsRF.RFw_rl */
        __dtBusInfo[36] = ssGetBusElementOffset(S, __shellsRFId, 1);
        __dtBusInfo[37] = 16*__dtSizeInfo[1];

        /* Get information for accessing shellsRF.RFw_fr */
        __dtBusInfo[38] = ssGetBusElementOffset(S, __shellsRFId, 2);
        __dtBusInfo[39] = 16*__dtSizeInfo[1];

        /* Get information for accessing shellsRF.RFw_fl */
        __dtBusInfo[40] = ssGetBusElementOffset(S, __shellsRFId, 3);
        __dtBusInfo[41] = 16*__dtSizeInfo[1];
...

        {
            ...
            sfun_init(p1, p2, p3, p4, p5, p6, p7, p8, p9, p10, p11, p12);
        }
    } /* if */
}
#endif
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
    if (IS_SIMULATION_TARGET(S)) {
        ...
        (void) memcpy(__u1BUS[0].RFw_rr, u1 +  __dtBusInfo[34],  __dtBusInfo[35]);
        (void) memcpy(__u1BUS[0].RFw_rl, u1 +  __dtBusInfo[36],  __dtBusInfo[37]);
        (void) memcpy(__u1BUS[0].RFw_fr, u1 +  __dtBusInfo[38],  __dtBusInfo[39]);
        (void) memcpy(__u1BUS[0].RFw_fl, u1 +  __dtBusInfo[40],  __dtBusInfo[41]);

        /* Call the legacy code function */
        sfun_out(__u1BUS, __y1BUS, p13, p14);
        ...
        (void) memcpy(y1 +  __dtBusInfo[24], &__y1BUS[0].rho_dot_rr,  __dtBusInfo[25]);
        (void) memcpy(y1 +  __dtBusInfo[26], &__y1BUS[0].rho_dot_rl,  __dtBusInfo[27]);
        (void) memcpy(y1 +  __dtBusInfo[28], &__y1BUS[0].rho_dot_fr,  __dtBusInfo[29]);
        (void) memcpy(y1 +  __dtBusInfo[30], &__y1BUS[0].rho_dot_fl,  __dtBusInfo[31]);
        (void) memcpy(y1 +  __dtBusInfo[32], &__y1BUS[0].in_mesh,  __dtBusInfo[33]);
    } /* if */
}
...
static void CheckDataTypes(SimStruct *S)
{
    ...
    {
        uint32_T chk[] = {2165091627, 2985902486, 1830311733, 3731962070};
        int_T status;
