...
#define MEX_ERROR_MESSAGE                                                     \
  ...
  "%                           FRICTION  : Triangleground face friction  %\n" \
  "%                         );                                          %\n" \
  "%   OUT = mex_triangleground( 'new',                                  %\n" \
  ...
  "%         mex_triangleground( 'setFriction', OBJ, FRICTION );         %\n" \
...
#define CMD "mex_triangleground( 'new', [, args] ): "
  MEX_ASSERT(nrhs == 1 || nrhs == 5 || nrhs == 11, CMD "expected 1, 5 or 11 inputs, nrhs = " << nrhs << '\n');
  ...
  if (nrhs == 5)
  {
    ...
    mwSize           rows1, cols1;
    matrix1_ptr = getMatrixPointer(arg_in_1, rows1, cols1, CMD "Error in reading first input matrix");
    ...
    mwSize           rows2, cols2;
    matrix2_ptr = getMatrixPointer(arg_in_2, rows2, cols2, CMD "Error in reading second input matrix");
    ...
    mwSize           rows3, cols3;
    matrix3_ptr = getMatrixPointer(arg_in_3, rows3, cols3, CMD "Error in reading third input matrix");
    MEX_ASSERT(rows3 == 3 || cols3 == 1, CMD "expected rows = 3 and cols = 1 found, rows = " << rows3 << ", cols = " << cols3 << '\n');
    x3 = matrix3_ptr[0];
    y3 = matrix3_ptr[1];
    z3 = matrix3_ptr[2];
    mu = getScalarValue(arg_in_4, CMD "Error in reading fourth input value");
  }
  else if (nrhs == 11)
  {
    ...
    mu = getScalarValue(arg_in_10, CMD "Error in reading fourth input value");
  }

  enve::triangleground *ptr = new enve::triangleground(acme::point(x1, y1, z1), acme::point(x2, y2, z2), acme::point(x3, y3, z3), mu);
...
#define CMD "mex_triangleground( 'setFriction', OBJ, FRICTION ): "
