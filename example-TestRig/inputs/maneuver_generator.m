...
inputs.x = timeseries(ones(l,1).*30,t);
...
inputs.z = timeseries(ones(l,1).*0.35,t);
...
inputs.theta_x = timeseries(sin(t).*pi/9,t);
...
inputs.omega = timeseries(ones(l,1)*1,t);


en_torque = 1; % enable torque input at the wheel
inputs.torque = timeseries(zeros(l,1),t);% wheel torque [Nm]
inputs.omega0 = 30/0.3130; % initial condition for omega in case of torque input
