function tirParams = TIR_read(FileNameLocation)%#codegen
...
% parameter.
%
% Syntax: [tirParams] = readTIR(FileNameLocation), where:
%
% - FileNameLocation may refer to a Magic Formula 6.1 - 6.2 tyre property file
%       (.tir)
...
for jj = 1:nlines
    ...
    if ~isempty(currentLine)
        ...
        % If equal exists
        if ~isempty(temp_equal)
            
            % If current line is not a title, comment or description
            currentLineNoSpaces = strtrim(currentLine);
            if (currentLineNoSpaces(1) ~= '[' && currentLineNoSpaces(1) ~= '$' && currentLineNoSpaces(1) ~= '{' && currentLineNoSpaces(1) ~= '!')
                ...
                if strcmp(ParamName,'QBZ6')
                    tirParams.QBZ6 = str2double(ParamValue);
                end
                ...
                if strcmp(ParamName,'QDRP2')
                    tirParams.QDRP2 = str2double(ParamValue);
                end
