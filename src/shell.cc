...
shell::shell(size_t size, real Rx, real Mx, real Ry, real My, real Ly)
...
void shell::resize(size_t size) {
#define CMD "enve::shell::resize(...): "

  ENVE_ASSERT(size > size_t(0), CMD "negative ribs number detected.");
  ...
  this->m_ribs.clear();
  this->m_ribs.reserve(200);
  this->m_out.clear();
  this->m_out.reserve(200);
  this->m_candidates.clear();
  this->m_candidates.reserve(200);
  ...
  for (size_t i = 0; i < size; ++i) {
    ...
    ENVE_ASSERT(ribR > size_t(0), CMD "negative rib radius detected.");
    ENVE_ASSERT(ribR == ribR, CMD "NaN rib radius detected.");
    ENVE_ASSERT(ribA == ribA, CMD "NaN rib angle detected.");
...
size_t shell::size(void) const { return this->m_ribs.size(); }
...
real shell::surfaceMaxRadius(void) const {
  return this->m_shape->surfaceMaxRadius();
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

real shell::surfaceMaxWidth(void) const {
  return this->m_shape->surfaceMaxWidth();
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

real shell::surfaceWidth(void) const { return this->m_shape->surfaceWidth(); }

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

bool shell::checkWidthBound(real y) const {
  return this->m_shape->checkWidthBound(y);
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

real shell::surfaceRadius(real y) const {
  return this->m_shape->surfaceRadius(y);
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

real shell::surfaceDerivative(real y, real tolerance) const {
  return this->m_shape->surfaceDerivative(y, tolerance);
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

real shell::surfaceAngle(real y, real tolerance) const {
  return this->m_shape->surfaceAngle(y, tolerance);
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

real shell::ribRadius(size_t i) const { return this->m_ribs[i].radius(); }

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

point shell::ribCenter(size_t i) const { return this->m_ribs[i].center(); }

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

real shell::ribWidth(size_t i) const { return this->m_ribs[i].width(); }

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

real shell::ribAngle(size_t i) const { return this->m_ribs[i].angle(); }
...
void shell::translate(vec3 const &vector) { this->m_affine.translate(vector); }

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

vec3 shell::translation(void) const { return this->m_affine.translation(); }

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

void shell::rotate(real angle, vec3 const &axis) {
  this->m_affine.rotate(angleaxis(angle, axis));
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

mat3 shell::rotation(void) const { return this->m_affine.rotation(); }

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

mat3 shell::linear(void) const { return this->m_affine.linear(); }

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

void shell::transform(affine const &pose) {
...
affine const &shell::transformation(void) const { return this->m_affine; }

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

bool shell::checkTransformation(mat4 const &pose, real tolerance) const {
...
bool shell::checkTransformation(affine const &pose, real tolerance) const {
  return this->checkTransformation(pose.matrix(), tolerance);
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

vec3 shell::x(void) const { return this->m_affine.linear().col(0); }

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

vec3 shell::y(void) const { return this->m_affine.linear().col(1); }

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

vec3 shell::z(void) const { return this->m_affine.linear().col(2); }

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

void shell::eulerAngles(vec3 &angles) const {
...
std::shared_ptr<aabb> shell::bbox(void) const { return this->m_bbox; }

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

void shell::updateBBox(void) {
...
bool shell::setup(ground::mesh const &ground, affine const &pose,
...
#define CMD "enve::shell::setup(...): "
  ...
  if (local_ground.size() < size_t(1)) {
    ...
    this->refineIntersection(local_ground,
                             method == "geometric" &&
    ...
    bool out = false;
    for (size_t i = 0; i < this->size(); ++i) {
      out = this->m_ribs[i].envelop(this->m_candidates[i], pose, method,
                                    this->m_out[i]) ||
            out;
    }
    return out;
...
bool shell::setup(ground::flat const &ground, affine const &pose,
                  std::string const method) {
#define CMD "enve::shell::setup(...): "
...
bool shell::sample(ground::mesh const &ground, point const &line_center,
                   point &contact_point) {
#define CMD "enve::shell::setup(...): "
...
void shell::contactPoint(point &point) const {
  point = (ZEROS_VEC3);
...
void shell::contactPoint(size_t i, point &point) const {
  point = this->m_out[i].point;
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

void shell::contactPoint(std::vector<point> &point) const {
  size_t size = this->size();
  point.resize(size);
  for (size_t i = 0; i < size; ++i) {
...
void shell::contactNormal(vec3 &normal) const {
  normal = ZEROS_VEC3;
  size_t size = this->size();
  real volume;
...
void shell::contactNormal(size_t i, vec3 &normal) const {
  normal = this->m_out[i].normal;
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

void shell::contactNormal(std::vector<vec3> &normal) const {
  size_t size = this->size();
  normal.resize(size);
  for (size_t i = 0; i < size; ++i) {
...
void shell::contactFriction(real &friction) const {
  friction = 0.0;
  size_t size = this->size();
  real volume_sum = 0.0;
  this->contactVolume(volume_sum);
...
void shell::contactFriction(size_t i, real &friction) const {
  friction = this->m_out[i].friction;
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

void shell::contactFriction(std::vector<real> &friction) const {
  size_t size = this->size();
...
void shell::contactDepth(real &depth) const {
  depth = 0.0;
  size_t size = this->size();
  real volume_sum = 0.0;
  this->contactVolume(volume_sum);
...
void shell::contactDepth(size_t i, real &depth) const {
  depth = this->m_out[i].depth;
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

void shell::contactDepth(std::vector<real> &depth) const {
  size_t size = this->size();
  depth.resize(size);
  for (size_t i = 0; i < size; ++i) {
...
void shell::contactArea(real &area) const {
  area = 0.0;
  for (size_t i = 0; i < this->size(); ++i) {
...
void shell::contactArea(size_t i, real &area) const {
  area = this->m_out[i].area;
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

void shell::contactArea(std::vector<real> &area) const {
  size_t size = this->size();
  area.resize(size);
  for (size_t i = 0; i < size; ++i) {
...
void shell::contactVolume(real &volume) const {
  volume = 0.0;
  for (size_t i = 0; i < this->size(); ++i) {
...
void shell::contactVolume(size_t i, real &volume) const {
  volume = this->m_out[i].volume;
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

void shell::contactVolume(std::vector<real> &volume) const {
  size_t size = this->size();
  volume.resize(size);
  for (size_t i = 0; i < size; ++i) {
...
void shell::contactPointAffine(affine &pose) const {
  point point;
  vec3 normal;
  ...
  pose.matrix() << x_vec, y_vec, normal, point,
      vec4(0.0, 0.0, 0.0, 1.0).transpose();
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

void shell::contactPointAffine(size_t i, affine &pose) const {
  vec3 x_vec((this->y().cross(this->m_out[i].normal)).normalized());
  vec3 y_vec((this->m_out[i].normal.cross(x_vec)).normalized());
  pose.matrix() << x_vec, y_vec, this->m_out[i].normal, this->m_out[i].point,
      vec4(0.0, 0.0, 0.0, 1.0).transpose();
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

void shell::contactPointAffine(std::vector<affine> &pose) const {
  for (size_t i = 0; i < this->size(); ++i) {
...
void shell::relativeAngles(vec3 &angles) const {
  affine pose;
...
void shell::relativeAngles(size_t i, vec3 &angles) const {
  affine pose;
...
void shell::relativeAngles(std::vector<vec3> &angles) const {
  for (size_t i = 0; i < this->size(); ++i) {
...
void shell::print(out_stream &os) const {
  point cpoint;
  ...
  os << " ------------ SHELL PRINT ------------" << std::endl << std::endl;
  ...
  for (size_t i = 0; i < point_vec.size(); ++i) {
    os << "Rib " << i << " - P = " << point_vec[i].transpose() << " m"
       << std::endl;
  }
  ...
  os << "Shell reference frame" << std::endl
     ...
     << "[Γ ß α]' = " << relative_angles.transpose() / PI << "pi rad"
     << std::endl
     << "Ribs relative pose angles" << std::endl;
  for (size_t i = 0; i < relative_angles_vec.size(); ++i) {
    os << "Rib " << i
       << " - [Γ ß α]' = " << relative_angles_vec[i].transpose() / PI
       << "pi rad" << std::endl;
  }
  os << std::endl;
...
void shell::refineIntersection(triangleground::vecptr const &local_ground,
                               bool refine) {
  size_t size = this->size();
